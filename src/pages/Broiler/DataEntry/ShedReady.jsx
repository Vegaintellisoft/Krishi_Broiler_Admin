...
const ShedReady = () => {
    ...
    // In real app, this comes from an API when Farmer is selected
    const farmerDB = {
    ...
    const fetchData = async () => {
        ...
        setTimeout(() => {
            setData([
                { id: 1, plant: 'Chennai Unit 1', farmer: 'FARM-001 | Krishnan', date: '2025-10-24', chickHouseCapacity: 5000, batchNo: 'B-2025-01', prevBatch: 'B-2024-12', excessHoused: 50 },
                { id: 2, plant: 'Chennai Unit 1', farmer: 'FARM-002 | Balaji', date: '2025-10-25', chickHouseCapacity: 4500, batchNo: 'B-2025-05', prevBatch: 'B-2024-11', excessHoused: 0 },
            ]);
            setIsLoading(false);
        }, 500);
    };
                        ...
                        <tbody className="divide-y font-poppins">
                            {paginatedData.length > 0 ? paginatedData.map((item, index) => (
                                <tr key={item.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="p-4 text-center opacity-65">{item.chickHouseCapacity}</td>
                                    <td className="p-4 text-center opacity-65">{item.excessHoused}</td>
                                    {(shedReadiness?.edit || shedReadiness?.delete) && (
                                        <td className="p-4 flex justify-center gap-4">
                                            {shedReadiness?.edit && <button onClick={() => openModal(item)} className="hover:text-orange-500"><FiEdit2 size={18} /></button>}
                                            {shedReadiness?.delete && <button onClick={() => handleDelete(item.id)} className="hover:text-red-500"><RiDeleteBin6Line size={18} /></button>}
                                        </td>
                                    )}
                                </tr>
                            )) : (
