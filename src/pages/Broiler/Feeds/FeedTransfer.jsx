...
const FeedTransfer = () => {
        ...
        setTimeout(() => {
            setData([
                ...
                },
                {
                    ...
                    plant: 'Chennai Unit 1',
                    transferType: 'RT',
                    fromFarmer: 'FARM-003 | Kumar',
                    toFarmer: 'FARM-001 | Krishnan',
                    material: 'Broiler Starter',
                    transferQty: 15,
                    vehicleNo: 'TN-01-ZZ-9876',
                    // Hidden fields
                    fromBatch: 'B-2025-08', fromBirdStock: 6000, fromAge: 2,
                    toBatch: 'B-2025-01', toBirdStock: 5000, toAge: 12,
                    iftCharges: 375, storageLoc: '0012'
                },
            ]);
            setIsLoading(false);
        }, 500);
    ...
    const filteredData = data.filter(item =>
        item.fromFarmer.toLowerCase().includes(searchQuery.toLowerCase()) ||
        item.toFarmer.toLowerCase().includes(searchQuery.toLowerCase()) ||
                        ...
                        <tbody className="divide-y font-poppins">
                            {paginatedData.length > 0 ? paginatedData.map((item, index) => (
                                <tr key={item.id} className="hover:bg-gray-50">
                                    ...
                                    <td className="p-4 opacity-65">{item.transferType}</td>
                                    <td className="p-4 opacity-65">{item.fromFarmer}</td>
                                    <td className="p-4 opacity-65">{item.toFarmer}</td>
                                    <td className="p-4 opacity-65">{item.material}</td>
                                    <td className="p-4 text-center opacity-65">{item.transferQty}</td>
                                    <td className="p-4 opacity-65">{item.vehicleNo}</td>
                            ...
                            <div className="flex flex-col gap-5">
                                ...
                                <div className="space-y-4 border p-4 rounded-lg bg-gray-50 h-fit">
                                    ...
                                    <div>
                                        ...
                                        <input type="date" name="date" value={formData.date} readOnly className="w-full p-2 bg-gray-100 border rounded text-sm text-gray-500" />
                                    </div>
                                    ...
                                    <div>
                                        ...
                                        <input type="text" name="vehicleNo" value={formData.vehicleNo} onChange={handleInputChange} className="w-full p-2 bg-white border rounded text-sm focus:ring-2 focus:ring-orange-500 outline-none" required placeholder="e.g. TN-01-AB-1234" />
                                    </div>

                                    <div>
                                        <label className="block text-xs font-bold text-gray-700 mb-1">IFT Charges <span className="text-red-500">*</span></label>
                                        <input type="number" name="iftCharges" value={formData.iftCharges} onChange={handleInputChange} className="w-full p-2 bg-white border rounded text-sm focus:ring-2 focus:ring-orange-500 outline-none" required placeholder="Enter Amount" />
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <div className="space-y-3 border p-4 rounded-lg bg-white">
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            <div><label className="text-[10px] text-gray-500">Batch</label><input value={formData.fromBatch} readOnly className="w-full p-1.5 bg-gray-100 border rounded text-xs" /></div>
                                            <div><label className="text-[10px] text-gray-500">Age</label><input value={formData.fromAge} readOnly className="w-full p-1.5 bg-gray-100 border rounded text-xs" /></div>
                                            <div className="col-span-2"><label className="text-[10px] text-gray-500">Bird Stock</label><input value={formData.fromBirdStock} readOnly className="w-full p-1.5 bg-gray-100 border rounded text-xs" /></div>
                                        </div>
                                    </div>
                                    ...
                                    <div className="space-y-3 border p-4 rounded-lg bg-white">
                                        ...
                                        <div className="grid grid-cols-2 gap-3">
                                            <div><label className="text-[10px] text-gray-500">Batch</label><input value={formData.toBatch} readOnly className="w-full p-1.5 bg-gray-100 border rounded text-xs" /></div>
                                            <div><label className="text-[10px] text-gray-500">Age</label><input value={formData.toAge} readOnly className="w-full p-1.5 bg-gray-100 border rounded text-xs" /></div>
                                            <div className="col-span-2"><label className="text-[10px] text-gray-500">Bird Stock</label><input value={formData.toBirdStock} readOnly className="w-full p-1.5 bg-gray-100 border rounded text-xs" /></div>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div className="space-y-4 border p-4 rounded-lg bg-gray-50 h-fit">
                                    ...
                                    <div>
                                        ...
                                        <input type="number" name="transferQty" value={formData.transferQty} onChange={handleInputChange} className="w-full p-2 bg-white border rounded text-sm focus:ring-2 focus:ring-orange-500 outline-none" required placeholder="Enter Qty" />
                                    </div>

                                    <div>
                                        <label className="block text-xs font-bold text-gray-700 mb-1">Storage Loc</label>
                                        <input type="text" name="storageLoc" value={formData.storageLoc} readOnly className="w-full p-2 bg-gray-200 border rounded text-sm text-gray-600 font-mono" />
                                    </div>
                                </div>
