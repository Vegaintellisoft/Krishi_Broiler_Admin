...
const DIVISION_CONFIG = {
    // Wagon uses the old, nested permissions structure (and corresponding module names)
    "Wagon": {
    ...
    // Broiler uses the new, flat permission structure (and corresponding module names)
    "Broiler": {
        defaultRoleName: "Broiler Supervisor (e.g., Broiler Head)",
        accessRules: [
            "StockLocation", "HetcheryMachine", "LineMaster", "RejectionReason", "ShedCapacity",
             "MortalityReason", "StandardBody", "MortalityIncentive", "MortalityDeduction", "medicineMaintain",
              "earnedRcMaster", "earnedRcMaster1", "vehicleTypeCost", "fcrGrade", "eggCodelist",
               "fcrGradeMaster1","broilerShedIncentive", "mortalityIncenPlant", "mortalityDeducPlant", "mortalityDeducPlant1",
                "mortalityDeducMainPlant", "broilerSalesEmpDefault", "broilerSalesRate","earnedRcMaster2", "earnedRcMaster3",
                "broilerUsers","adminPage"
        ],
    },
    // Breeder uses the new, flat permission structure
    "Breeder": {
...
export default function Roles() {
    ...
    const initializeDivisionAccessRules = useCallback((initialCategory = INITIAL_CATEGORY, existingRole = {}) => {
        ...
        config.accessRules.forEach(menuItem => {
            const actions = WAGON_SPECIAL_ACTIONS[menuItem] || DEFAULT_ACCESS_ACTIONS;
    ...
    const handleDivisionAccessChange = (menuItem, action, isChecked) => {
        setFormData((prev) => {
            ...
            // Implementation of Cascading Logic:
            // If the 'show' (or 'list') permission is turned OFF, 
            // all other associated permissions for that module must also be turned OFF.
            if (!isChecked && (action === "show" || action === "list")) {
                const actionsToClear = (WAGON_SPECIAL_ACTIONS[menuItem] || DEFAULT_ACCESS_ACTIONS).filter(a => a !== action);
    ...
    const handleSubmit = async (e) => {
        ...
        // Initial payload contains only basic moderator data
        const payload = {
        ...
        // Iterate over all flat keys in formData
        Object.keys(formData).forEach(key => {
            if (key.includes('_')) {
                const [moduleName, action] = key.split('_');

                if (accessRules.includes(moduleName)) {

                    const viewActions = ["show", "list"];
                        ...
                        <div className="h-[75vh] space-y-8 overflow-y-auto p-8">
                            ...
                            <div>
                                ...
                                <input
                                    ...
                                    required
                                    className="w-full rounded-lg border bg-gray-50 p-3 text-base focus:outline-none focus:ring-2 focus:ring-orange-500"
                            ...
                            {/* --- Division Access Rules Table (The new permission UI) --- */}
                            <div>
                                    ...
                                    <table className="min-w-full text-sm text-center border-collapse">
                                        <thead className="bg-gray-800 text-white sticky top-0">
                                            <tr>
                                                ...
                                                {/* Dynamically render headers based on all possible actions for the current ruleset */}
                                                {currentAccessRules.reduce((acc, moduleName) => {
                                                    const actions = WAGON_SPECIAL_ACTIONS[moduleName] || DEFAULT_ACCESS_ACTIONS;
                                                    actions.forEach(action => {
                                        ...
                                        <tbody>
                                            {currentAccessRules.map((menuItem, index) => {
                                                const moduleActions = WAGON_SPECIAL_ACTIONS[menuItem] || DEFAULT_ACCESS_ACTIONS;
                                                ...
                                                const showKey = `${menuItem}_show`;
                                                // Check if the 'show' key is defined and set to true
                                                const isShowEnabled = formData[showKey] === true;
                                                ...
                                                return (
                                                    ...
                                                    >
                                                        <td className="text-left py-3 capitalize px-6 font-semibold text-gray-800">
                                                            {menuItem}
                                                        </td>
                                                        {allPossibleActions.map((action) => {
                                                            ...
                                                            // Determine if the toggle should be disabled based on cascading rule
                                                            const isSubPermission = !['show', 'list', 'view'].includes(action);
                                                            ...
                                                            // Visually force unchecked if disabled by rule, but keep original formData for change detection
                                                            if (isDisabledByRule) {
                                                                isChecked = false;
                                                            }
                                                                    ...
                                                                    {isModuleAction ? (
                                                                        <label className={`relative inline-flex items-center cursor-pointer ${isDisabledFinal ? 'opacity-50 cursor-not-allowed' : ''}`}>
                                                                            <input
                                                                                ...
                                                                                onChange={(e) => {
                                                                                    handleDivisionAccessChange(menuItem, action, e.target.checked);
                                                                                }}
                                                                                checked={isChecked} // Use the potentially forced-unchecked value
                                                                        ...
                                                                        <span className="text-gray-300">—</span> // Placeholder for non-applicable action
                                                                    )}
                                                                </td>
                                                            );
                                                        })}
                                            ...
                                            {currentAccessRules.length === 0 && (
                                                <tr>
                                                    <td colSpan={7} className="py-8 text-gray-500">No specific access rules configured for this division.</td>
                                                </tr>
                                            )}
                                        </tbody>
                                    </table>
                            ...
                            {/* --- END Division Access Rules Table --- */}
